...
// elements-tier story (rule 2b): react-day-picker isn't decomposable into `useRender` styled parts, so
// the `elements/calendar` styled layer is the `calendarClassNames` map — not a component (hence a
// component-less meta). This in-tier story grafts that styled contract onto react-day-picker's
// `DayPicker` behavior directly (`classNames={calendarClassNames}`), the react-day-picker analog of
// wiring Base UI behavior onto a styled `elements` element — no `components` import, react-day-picker's
// default nav chevron. The `components/calendar` ready-made swaps in propel's lucide chevrons.
const meta = {
