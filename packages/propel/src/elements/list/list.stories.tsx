...
// `useRender` elements; the roving-tabindex behavior grafts onto them from the `base/composite`
// primitive via `render` (an `elements`/`base` story may import `base`). `List` is role-flexible; this
// story uses `role="toolbar"` (the canonical roving-controls role) so axe passes while showing the
// chrome. The production role — navigation tree vs region — is settled at composition time.
const meta = {
  title: "Elements/List",
  component: List,
  subcomponents: {
